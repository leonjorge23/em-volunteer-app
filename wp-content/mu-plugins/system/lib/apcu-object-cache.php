 ...
 * APCu Object Cache.
 *
 * Based on APCu Object Cache Backend by Pierre Schmitz.
...
function _mwp_system_apcu_is_blocked_session() {
	...
	return (bool) preg_grep( $pattern, array_keys( $_COOKIE ) );

}

function _mwp_system_apcu_is_blocked_page() {

	return ( defined( 'WP_ADMIN' ) || defined( 'DOING_AJAX' ) || defined( 'XMLRPC_REQUEST' ) || filter_input( INPUT_GET, 'nocache' ) || 'wp-login.php' === basename( $_SERVER['SCRIPT_FILENAME'] ) );

}

if ( function_exists( 'apcu_fetch' ) && 'cli' !== php_sapi_name() && ! WP_DEBUG && ! _mwp_system_apcu_is_blocked_session() && ! _mwp_system_apcu_is_blocked_page() ) :
	 ...
	 * @return bool
	 */
	function _mwp_system_apcu_save_transient( $transient, $value, $expire, $site = false ) {
	...
	function _mwp_system_apcu_prune_transients() {
	 ...
	 * @return void
	 */
	function _mwp_system_apcu_init_sync_cache() {
		...
		$master = $wpdb->get_var( "SELECT option_value FROM `{$wpdb->options}` WHERE option_name = 'mwp_system_last_object_cache_flush';" );
		...
		$local = wp_cache_get( 'mwp_system_last_object_cache_flush' );

		if ( false === $local || (int) $local < (int) $master ) {

			wp_cache_flush( true );

			wp_cache_set( 'mwp_system_last_object_cache_flush', time() );
	...
	function wp_cache_add( $key, $data, $group = '', $expire = 0 ) {
		...
		if ( 'transient' === $group ) {
			...
			_mwp_system_apcu_save_transient( $key, $data, $expire );

			return $wp_object_cache->add( "_transient_$key", $data, 'options', $expire );

		} elseif ( 'site-transient' == $group ) {

			_mwp_system_apcu_save_transient( $key, $data, $expire, true );
	...
	function wp_cache_flush( $local_flush = false ) {
		...
		if ( ! $local_flush ) {
			...
			if ( ! empty( $wpdb ) && $wpdb instanceof wpdb ) {
				...
				$flag = $wpdb->suppress_errors;
				$wpdb->suppress_errors( true );
				$wpdb->query( "INSERT INTO `{$wpdb->options}` (`option_name`, `option_value`, `autoload`) VALUES ( 'mwp_system_last_object_cache_flush', UNIX_TIMESTAMP( NOW() ), 'no' ) ON DUPLICATE KEY UPDATE `option_name` = VALUES ( `option_name` ), `option_value` = VALUES ( `option_value` ), `autoload` = VALUES ( `autoload` );" );
				$wpdb->suppress_errors( $flag );
	...
	function wp_cache_init() {
		...
		if ( mt_rand( 1, 100 ) == 42 ) {
			...
			_mwp_system_apcu_prune_transients();

		}

		add_action( 'muplugins_loaded', '_mwp_system_apcu_init_sync_cache' );
	...
	function wp_cache_set( $key, $data, $group = '', $expire = 0 ) {
		...
		if ( defined( 'WP_INSTALLING' ) === false ) {
			...
			if ( 'transient' === $group ) {
				...
				return _mwp_system_apcu_save_transient( $key, $data, $expire );

			} elseif ( 'site-transient' === $group ) {

				return _mwp_system_apcu_save_transient( $key, $data, $expire, true );
	...
	class MWP_System_APCu_Object_Cache {
	...
	if ( function_exists( 'apcu_inc' ) ) {
		...
		class APCu_Object_Cache extends MWP_System_APCu_Object_Cache {
